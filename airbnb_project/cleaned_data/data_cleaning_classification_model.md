## Data Cleaning & Preprocessing (Superhost Classification Model)
...
- **Behavioral variables (e.g., revenue, activity)**: Filled with **0** (indicates no activity)
- **Review recency variables**: Filled with **9999** (represents listings with no reviews)
- **Host listing count**:
  → Imputed with **1 (mode)** instead of median (better reflects typical small-scale hosts)
